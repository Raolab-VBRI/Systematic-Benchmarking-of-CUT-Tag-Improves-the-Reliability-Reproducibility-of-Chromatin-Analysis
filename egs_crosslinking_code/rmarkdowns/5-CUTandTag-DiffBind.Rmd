...
```{r Create a dataframe with MACS2 metadata}
  ...
  sampleList <- sampleList[grep(paste0(target, "_"), sampleList)]
    ...
    for(hist in sampleList){
      histInfo = strsplit(hist, "_")[[1]]
       ...
       macs_ext <- ".macs2.nolambda_peaks.narrowPeak"
       peak_type <- "narrow"
...
This first heatmap is based on the peak files from MACS2 which are used to create "occupancy scores." Essentially, peaks with more overlaps are considered to be more closely correlated.
```{r dba heatmap}
    ...
    for(cond in c("0nMEGS", "200nMEGS", "01pF")){
      ...
      if (!is.null(consensus_list[[cond]])) {
        rtracklayer::export.bed(consensus_list[[cond]],
                              con = file.path(out_dir, paste0(cond, "_", target, "_consensus.macs2.bed")))
