...
## MACS2 Peak Calling 
### Cut off analysis
Determine cutoff empirically using MACS2 --cutoff-analysis
```{cat MACS2 Cutoff Analysis, engine.opts = list(file = 'scripts/9-MACS2_cutoff.sh')}
#!/bin/bash
...
peakPath="${projPath}/peakCalling/MACS2/cutoff_analysis"
...
sbatch scripts/9-MACS2_cutoff.sh
```
...
```{r}
ggsave(filename = "EGSx_CnT_MACS2_elbowPlot_All.pdf", 
       plot = all_elbow,
       width = 7, height = 4,
       path = figure_dir)
```
...
```{cat MACS2 peak calling, engine.opts = list(file = 'scripts/10-MACS2_callPeaks.sh')}
#!/bin/bash
...
projPath="/scratch/g/srrao/josiah_ociaml3/EGSx_CUTnTag"
bamPath="${projPath}/alignment/bam/hg38_bam"
peakPath="${projPath}/peakCalling/MACS2"
...
sbatch scripts/10-MACS2_callPeaks.sh
```
...
GetPeakN <- function( 
  ...
  for(hist in samples){
    ...
    broadPath <- file.path(peakPath, paste0(hist, gsub("narrow", "broad", peakExt)))
      ...
      # Handle broad peak files from MACS2   
...
macs_peakNumData <- GetPeakN(samples = bulk_sampleList,
         sampleDelimiter = "_",
         peakPath = macs_dir,
         peakExt = ".macs2.nolambda_peaks.narrowPeak",
         peakName = "macs2Peaks")
```
...
peakinfo_means <- peakinfo %>%
  ...
  summarise(
    mean_peaks = mean(macs2Peaks, na.rm = TRUE),
    median_peaks = median(macs2Peaks, na.rm = TRUE),
    sd_peaks = sd(macs2Peaks, na.rm = TRUE),
    n_samples = n(),
    .groups = 'drop'
  )
```
...
for (pair in pairwise_comparisons) {
  ...
  data1 <- peakinfo_clean$macs2Peaks[peakinfo_clean$crosslinking == group1]
  data2 <- peakinfo_clean$macs2Peaks[peakinfo_clean$crosslinking == group2]
...
peakN_fig1a <- peakinfo %>%
  ...
  ggplot(aes(x = crosslinking, y = macs2Peaks)) +
...
for (target in tarL) {
  ...
  for (type in macspeakType) {
    ...
    for (geno in genL) {
      for (rep in repL) {
        # Build file path for macs2 peaks
        peak_path <- file.path(macs_dir, paste0(geno, "_", target, "_", rep, ".macs2.nolambda_peaks.", type, "Peak"))
        ...
        if (file.exists(peak_path)) {
          ...
          # Read macs2 peak file as a table
          peakInfo <- read.table(peak_path, header = FALSE, fill = TRUE)
          
          # Build a granges object from the macs2 peak table
