...
peak_dir <- file.path("peakCalling", "MACS2")
...
for(cond in c("_0nMEGS", "_200nMEGS", "_01pF")){
  ...
  for(sample in list.files(path = peak_dir, pattern = paste0(cond, "-[A-Z]\\.macs2\\.nolambda_peaks\\.narrowPeak$"))){
...
nox_consensus <- read_delim(file = file.path(consensus_dir, "0nMEGS_CTCF_consensus.macs2.bed"),
                            delim = "\t",
                            col_names = c("Chr", "Start", "End", "peakID", "conc_NTC")) %>%
                              makeGRangesFromDataFrame(keep.extra.columns = TRUE)

egs_consensus <- read_delim(file = file.path(consensus_dir, "200nMEGS_CTCF_consensus.macs2.bed"),
                            delim = "\t",
                            col_names = c("Chr", "Start", "End", "peakID", "conc_NTC")) %>%
                              makeGRangesFromDataFrame(keep.extra.columns = TRUE)


pf_consensus <- read_delim(file = file.path(consensus_dir, "01pF_CTCF_consensus.macs2.bed"),
                            delim = "\t",
